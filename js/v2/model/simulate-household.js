...
export function simulateHousehold(model,{data,taxTables}={}){
  ...
  const activePersonIds=new Set(people.map(person=>person.id)),activeCareers=model.careers.filter(stage=>activePersonIds.has(stage.personId)),activeServiceHistories=model.serviceHistories.filter(history=>activePersonIds.has(history.personId));
  ...
  for(const year of years){
    ...
    const records=produceCompensationRecords({stages:activeCareers,year,serviceHistories:activeServiceHistories,data});
    const retirement=retirementContributionsForYear({records,stages:activeCareers,year,serviceHistories:activeServiceHistories,policies:model.retirementPolicies,brsRules:data.brsRules});
