...
test('run identity includes all reproducibility fingerprints and approved versions',()=>{const {model}=goldenProductionFixture(),manifest={engineVersion:'v2.0.3',outputContractVersion:'2.0.2',inputHash:'input',assumptionsHash:'assumptions',dataFingerprint:'data',outputHash:'output',runId:'run'};const html=identityPanel(model,manifest);for(const value of ['v2.0.3','2.0.2','Input hash','Assumptions hash','Data fingerprint','Output fingerprint'])assert.match(html,new RegExp(value));assert.equal(model.modelVersion,'2.0.0');assert.equal(model.schemaVersion,'2.0.0');});
