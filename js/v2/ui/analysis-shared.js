...
export function jumpCandidates(simulation,model){if(model.extensions?.production?.template?.name==='Life Plan'){const military=model.careers.find(c=>c.careerType==='military'),combined=model.plannedEvents.find(e=>e.type==='household.combine'),possible=simulation.years.find(r=>r.goals?.some(g=>g.possible)),start=simulation.years[0].year,end=simulation.years.at(-1).year;return [{year:Number(military?.startDate?.slice(0,4)),label:'JAG start'},{year:Number(combined?.date?.slice(0,4)),label:'Household'},{year:possible?.year,label:'Home possible'},{year:Number(military?.endDate?.slice(0,4)),label:'Service transition'}].filter(x=>Number.isInteger(x.year)&&x.year>=start&&x.year<=end).sort((a,b)=>a.year-b.year);} const first=simulation.years[0],last=simulation.years.at(-1),items=simulation.timeline.filter(item=>['career','household','property'].includes(item.category)||(item.category==='goal'&&['possible','comfortable','executed'].includes(item.status)));const seen=new Set(),result=[];for(const item of items){const year=Number(String(item.realizedDate??item.plannedDate??'').slice(0,4));if(year<first.year||year>last.year||seen.has(year))continue;seen.add(year);result.push({year,label:item.category==='career'?'Career':item.category==='household'?'Household':item.category==='property'?'Property':'Goal'});if(result.length===4)break;}return result.length?result:[{year:first.year,label:`Age ${referenceAge(first,model)}`},{year:last.year,label:`Age ${referenceAge(last,model)}`}];}
