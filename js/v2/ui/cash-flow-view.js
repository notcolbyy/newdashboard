...
export function cashFlowMarkup({row,simulation,model}){
  ...
  return `<section class="view analysis-view" aria-labelledby="cash-flow-title">${periodExplorerMarkup({row,simulation,model,title:'Cash flow period'})}
    ...
    <section class="cash-hero" aria-label="Selected-year cash flow summary">
      ...
      ${summaryMetric('Household spending baseline',data.spending.totalHouseholdSpendingCents,'Property replacement costs are shown below')}
...
export function cashBridgeSvg(data){
  const items=[['Income',data.income.totalCashIncomeCents,'inflow'],['Taxes',data.taxes.totalTaxCents,'outflow'],['Spending',data.spending.totalHouseholdSpendingCents,'outflow'],['Debt',data.debt.debtServiceCents,'outflow'],['FCF',data.freeCashFlowCents,data.freeCashFlowCents<0?'negative':'result']],max=Math.max(1,...items.map(x=>Math.abs(x[1]))),width=780,left=112,right=104,barWidth=width-left-right;
  return `<figure class="cash-bridge"><svg viewBox="0 0 ${width} 273" role="img" aria-labelledby="cash-bridge-title cash-bridge-desc"><title id="cash-bridge-title">Annual cash flow bridge</title><desc id="cash-bridge-desc">Income ${formatCurrency(items[0][1])}, taxes ${formatCurrency(items[1][1])}, spending ${formatCurrency(items[2][1])}, debt service ${formatCurrency(items[3][1])}, and free cash flow ${formatCurrency(items[4][1])}.</desc>${items.map(([label,value,kind],i)=>{const y=10+i*43,w=Math.max(value===0?0:3,Math.abs(value)/max*barWidth);return`<text x="0" y="${y+19}" class="bridge-label">${label}</text><rect x="${left}" y="${y}" width="${w}" height="26" rx="5" class="bridge-bar ${kind}"/><text x="${width-2}" y="${y+19}" text-anchor="end" class="bridge-value">${escapeHtml(formatCurrency(value,{compact:true}))}</text>`;}).join('')}</svg><figcaption>These are statement components, not an additive bridge: rental financing is already netted in distributions and property costs replace generic housing. Retirement contributions are transfers. Principal reduces cash and debt, not net worth.</figcaption></figure>`;
}
